...
class VendorComparePage extends StatelessWidget {
              ...
              child: SingleChildScrollView(
                child: Padding(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      DataColumn(
                        label: Text('', style: GoogleFonts.montserrat(fontWeight: FontWeight.bold)),
                      ),
                    ...
                    rows: [
                      _buildRow('Location', vendors.map((v) => (v['vendor_location'] ?? '-').toString()).toList()),
                      _buildRow('Price', vendors.map((v) {
                        final price = v['vendor_price'] ?? v['vendor_estimated_price'] ?? '-';
                        return price.toString().isEmpty ? '-' : price.toString();
                      }).toList()),
                      _buildRow('Style', vendors.map((v) => (v['style_keywords'] ?? '-').toString()).toList()),
                      _buildRow('Email', vendors.map((v) => (v['contact_email'] ?? '-').toString()).toList()),
                      _buildRow('Phone', vendors.map((v) => (v['contact_phone'] ?? '-').toString()).toList()),
                    ],
                  ),
                ),
              ),
  ...
  DataRow _buildRow(String label, List<String> values) {
    return DataRow(cells: [
      DataCell(Text(label,
          style: GoogleFonts.montserrat(fontSize: 12, fontWeight: FontWeight.w600, color: const Color(0xFF7B3F61)))),
